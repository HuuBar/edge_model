...
"""
条件依赖数据重建 V3 —— 修复超时问题
核心改进: timeout=300s + max_tokens=8192 + 每次3条 + 截断内容恢复

用法:
    python generate_condition_v3.py --all
    python generate_condition_v3.py --batch 1 --count 40
"""
        ...
        try:
            resp = requests.post(
                ...
                json={
                    ...
                    "messages": [
                        {"role": "system", "content": "You generate WiFi customer service training data. Output ONLY a valid JSON array. No markdown code blocks, no extra text. Always complete all requested records."},
...
def parse_json(text: str) -> list:
    ...
    print(f"    JSON解析失败，文本长度{text[:200]}")
    return []
...
def validate(r: dict, idx: int):
    ...
    if not isinstance(react, list):
        errors.append(f"[{idx}] react not array")
        return errors
    if len(react) < 2:
    ...
    for si, step in enumerate(react):
        ...
        if si < len(react)-1 and not c.get("tool_name"):
            errors.append(f"[{idx}] step{si} missing tool_name")
        if si == len(react)-1 and not c.get("final_answer"):
            errors.append(f"[{idx}] last step missing final_answer")
    intent = r.get("intent_label_en")
    if intent and not isinstance(intent, dict):
        errors.append(f"[{idx}] intent not object")
...
2. intent_label_en MUST have task_decomposition array with task_num/intent/sub_task
3. categories MUST be array of strings like ["tool1", "tool2"]
4. query_cn must be natural Chinese, NO emotion prefix
5. Output ONLY JSON array, no markdown, no explanation
6. Generate ALL {n} records, do not truncate

Generate {n} records:"""

    result = call_llm(prompt, temperature=temp, max_tokens=8192, timeout=300)
...
def main():
    ...
    for bn in batches:
        ...
        save(records, bn)
        if bn < max(batches): time.sleep(5)
